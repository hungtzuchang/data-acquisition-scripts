...
for i in range(0,NumCycles):
    ...
    Sipumponsort=Sipumpon.sort_values('StageDelayAC_800',ascending=True)
    pumpon_numbers=np.r_[pumpon_numbers,Sipumponsort['ScanNumber']]
    Sipumpoffsort=Sipumpoff.sort_values('StageDelayAC_800',ascending=True)
    pumpoff_numbers=np.r_[pumpoff_numbers,Sipumpoffsort['ScanNumber']]
    
timeaxis=np.array(Sipumpoffsort['StageDelayAC_800']) 
